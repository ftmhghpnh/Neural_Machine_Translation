...
def _init_transformer(opts: argparse.Namespace,
        dataloader: a2_dataloader.HansardDataLoader
        ) -> a2_transformer.EncoderDecoder:
    import a2_transformer
    from a2_transformer import Encoder, EncoderLayer, Decoder, DecoderLayer, Embeddings, Generator

    src_vocab_len = dataloader.dataset.source_vocab_size
    tgt_vocab_len = dataloader.dataset.target_vocab_size
    N = opts.encoder_num_hidden_layers
    d_model = opts.word_embedding_size
    d_ff = 2048
    h = opts.heads  # default = 4, same used in mh_attn
    dropout = opts.encoder_dropout

    c = copy.deepcopy
    attn = a2_transformer.MultiHeadAttention(h, d_model, opts=opts)
    ff = a2_transformer.FeedForwardSubLayer(d_model, d_ff, dropout)
    pe = a2_transformer.PositionalEncoding(d_model, dropout)

    model = a2_transformer.EncoderDecoder(
        Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), N),
        Decoder(DecoderLayer(d_model, self_attn=c(attn), cross_attn=c(attn), feed_forward=c(ff), dropout=dropout), N),
        torch.nn.Sequential(Embeddings(d_model, src_vocab_len), c(pe)),
        torch.nn.Sequential(Embeddings(d_model, tgt_vocab_len), c(pe)),
...
def init(opts: argparse.Namespace, dataloader: a2_dataloader.HansardDataLoader) -> torch.nn.Module:
    if opts.with_transformer:
        return _init_transformer(opts, dataloader)
    else:
        return _init(opts,dataloader)
...
def train(opts: argparse.Namespace):
    ...
    train_dataloader = a2_dataloader.HansardDataLoader(
        opts.training_dir, french_word2id, english_word2id, opts.source_lang, train_prefixes,
        batch_size=opts.batch_size,
        pin_memory=(opts.device.type == 'cuda'),
        num_workers=1,
        arch_type="transformer" if opts.with_transformer else "seq2seq",
    ...
    dev_dataloader = a2_dataloader.HansardDataLoader(
        opts.training_dir, french_word2id, english_word2id, opts.source_lang, dev_prefixes,
        batch_size=opts.batch_size,
        pin_memory=(opts.device.type == 'cuda'),
        num_workers=1,
        arch_type="transformer" if opts.with_transformer else "seq2seq",
    ...
    if not opts.with_transformer:
        ...
        while epoch <= max_epochs and num_poor < patience:
            model.train()
            print(f"[Device:{opts.device}] Epoch {epoch} Training ====", flush=True)
            loss, train_state = a2_transformer.train_for_epoch(model, train_dataloader,
                optimizer,
                opts.device,
                mode="train+log",
            )
            if is_main_process:
                model.cpu()
                file_path = "%s%.2d.pt" % (opts.model_path, epoch)
                with smart_open(file_path, 'wb') as model_file:
                    torch.save(model.state_dict(), model_file)
            torch.cuda.empty_cache()

            print(f"[Device:{opts.device}] Epoch {epoch} Validation ====", flush=True)
            model.eval()
            with torch.no_grad():
                bleu = a2_training_and_testing.compute_average_bleu_over_dataset_using_transformer(
                    model, dev_dataloader,
                    dev_dataloader.dataset.target_sos,
                    dev_dataloader.dataset.target_eos,
                    opts.device,
...
def test(opts: argparse.Namespace):
    ...
    #TODO: Coalesce thsi if/else block here once transformer is tested:
    if not opts.with_transformer:
        ...
        model = _init_transformer(opts, dataloader)
