...
class Encoder(EncoderBase):
    ...
    def init_submodules(self):
        ...
        if self.cell_type == 'rnn':
            self.rnn = torch.nn.RNN(input_size=self.word_embedding_size, hidden_size=self.hidden_state_size, 
                                    num_layers=self.num_hidden_layers, dropout=self.dropout,
                                   bidirectional=True)
    ...
    def forward_pass(
        ...
        seq_hidden = self.get_all_hidden_states(embedding, source_x_lens, h_pad)
        return seq_hidden
...
#         assert False, "Fill me"
    ...
    def get_all_hidden_states(
        ...
        # assert False, "Fill me"
        packed = torch.nn.utils.rnn.pack_padded_sequence(x, source_x_lens.cpu(), batch_first=False, enforce_sorted=False)
        h0 = torch.randn(2 * self.num_hidden_layers, self.word_embedding_size, self.hidden_state_size, device=x.device)
        if self.cell_type == 'rnn':
            output, hn = self.rnn(packed, h0)
        elif self.cell_type == 'lstm':
            c0 = torch.randn(2 * self.num_hidden_layers, self.word_embedding_size, self.hidden_state_size, device=x.device)
            output, (hn, cn) = self.rnn(packed, (h0, c0))
        else:
            raise NotImplementedError()
        
        unpacked_output, lens_unpacked = torch.nn.utils.rnn.pad_packed_sequence(output, batch_first=False, padding_value=h_pad)
...
class DecoderWithoutAttention(DecoderBase):
    ...
    def forward_pass(
        ...
        # LSTM output is a tuple
        if self.cell_type == 'lstm':
            return self.get_current_logits(h_tilde_t[0]), h_tilde_t
        return self.get_current_logits(h_tilde_t), h_tilde_t
    ...
    def get_first_hidden_state(
        ...
        # assert False, "Fill me"
        forward = h[source_x_lens - 1, torch.arange(h.shape[1]), 0:self.hidden_state_size//2]
        backward = h[0, range(h.shape[1]), self.hidden_state_size//2:self.hidden_state_size]
...
class DecoderWithAttention(DecoderWithoutAttention):
    ...
    def get_first_hidden_state(
        ...
        # assert False, "Fill me"
        return torch.zeros((h.shape[1], h.shape[2]), device=h.device)
    ...
    def get_current_rnn_input(
        ...
        return torch.cat((c_t, xtilde_t), dim=-1)
    ...
    def attend(
        ...
        '''
        # assert False, "Fill me"
        ### here do not condition over the h, since expect to be handled in other parts
        ### if there is a bug, here can be checked!
        alpha_t = self.get_attention_weights(htilde_t, h, source_x_lens)
        c_t = torch.mul(alpha_t.unsqueeze(-1), h)
        return torch.sum(c_t, dim=0)
    ...
    def get_attention_scores(
        ...
        return torch.nn.functional.cosine_similarity(h, htilde_t.unsqueeze(0), dim=-1)
...
class DecoderWithMultiHeadAttention(DecoderWithAttention):
    ...
    def attend(
        ...
        # assert False, "Fill me" ## not complete yet
        size_of_partitions = self.hidden_state_size // self.heads
        S = h.shape[0]
        M = h.shape[1]
        h = self.W(h)
        h = h.view(S, -1, size_of_partitions)

        if self.cell_type == 'lstm':
            htilde_t = htilde_t[0]
        htilde_t = self.Wtilde(htilde_t)
        htilde_t = htilde_t.view(-1, size_of_partitions)
                
        c_t = super().attend(htilde_t, h, source_x_lens.repeat_interleave(repeats=self.heads))
        return self.Q(c_t.view(M, -1))
...
#         assert False, "Fill me"
    ...
    def translate(self, input_sentence):
        ...
        input_length = len(tokenized_sentence)
        
        token_id_seq = []
        for token in tokenized_sentence:
            token_id_seq.append(self.dataset.source_word2id.get(token, self.dataset.source_unk))
        
        output = self(torch.LongTensor([token_id_seq]).T, torch.LongTensor([input_length]))
        output_candidate = output[:, :, 0]
        translation = []
        for output_token in output_candidate:
            output_token = output_token.item()
            if output_token == self.target_sos:
                continue
            elif output_token == self.target_eos:
                break
            translation.append(self.dataset.target_id2word[output_token])
...
#         assert False, "Fill me"
        ...
        htilde_tm1 = None
        all_steps_logits = []
        for target_step in range(target_y.shape[0] - 1):
            logits, htilde_tm1 = self.decoder.forward(target_y[target_step, :], htilde_tm1, h, source_x_lens)
            all_steps_logits.append(logits)
        return torch.stack(all_steps_logits, dim=0)
    ...
    def update_beam(
        ...
        #   b_t_0 (first output) is of shape (B, K, 2 * H) or a tuple of two of
        #      those (LSTM)
        #   b_t_1 (second output) is of shape (t + 1, B, K)
        #   logpb_t (third output) is of shape (B, K)
        #
        # Hints:
...
#         assert False, "Fill me"
        B = logpb_tm1.shape[0]
        extended_paths = logpb_tm1.unsqueeze(-1) + logpy_t
        logpb_t, topk_indices = torch.topk(extended_paths.view(B, -1), k=self.beam_width, dim=1)
        selected_paths = torch.div(topk_indices, self.target_vocab_size, rounding_mode='floor')
        selected_vocabs = topk_indices % self.target_vocab_size
        
        if self.cell_type == 'lstm':
            b_t_0 = (torch.gather(htilde_t[0], dim=1, index=selected_paths.unsqueeze(-1).expand_as(htilde_t[0])), torch.gather(htilde_t[1], dim=1, index=selected_paths.unsqueeze(-1).expand_as(htilde_t[1])))
        else:
            b_t_0 = torch.gather(htilde_t, dim=1, index=selected_paths.unsqueeze(-1).expand_as(htilde_t))
        
        selected_sequences = torch.gather(b_tm1_1, dim=2, index=selected_paths.unsqueeze(0).expand_as(b_tm1_1))
        b_t_1 = torch.cat((selected_sequences, selected_vocabs.unsqueeze(0)), dim=0)
