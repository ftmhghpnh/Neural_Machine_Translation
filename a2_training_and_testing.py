...
#     assert False, "Fill me"
    loss_function = torch.nn.CrossEntropyLoss(ignore_index=dataloader.pad_id)
    total_loss = 0
    total_num_seqs = 0
    for source_x, source_x_lens, target_y in tqdm(dataloader):
        ...
        target_y = target_y[1:]
        target_mask = model.get_target_padding_mask(target_y)
        target_y = target_y.masked_fill(target_mask, dataloader.pad_id)
        
        logits = torch.flatten(logits, start_dim=0, end_dim=1)
        target_y = torch.flatten(target_y)
        
        loss = loss_function(logits, target_y)
        total_loss += loss.item()
        total_num_seqs += source_x.shape[1] # batch_size
        
        loss.backward()
        
        optimizer.step()
        
        del source_x, source_x_lens, target_y, logits, loss
        
    return total_loss / total_num_seqs
...
#     assert False, "Fill me"
    total_bleu = 0
    for seq_num in range(target_y_ref.shape[1]):
        reference_seq = [token for token in target_y_ref[:, seq_num].tolist() if (token not in [target_sos, target_eos])]
        candidate_seq = [token for token in target_y_cand[:, seq_num].tolist() if (token not in [target_sos, target_eos])]
        total_bleu += a2_bleu_score.BLEU_score(reference_seq, candidate_seq, 4)
    return total_bleu
...
#     assert False, "Fill me"
    ...
    for source_x, source_x_lens, target_y in tqdm(dataloader):
        ...
        total_bleu += compute_batch_total_bleu(target_y, target_y_cand, target_sos, target_eos)
        total_num_seqs += source_x.shape[1]
    return total_bleu / total_num_seqs
